"""
Contains all code for anchored MLPs (Pearce et al. 2018)
...
class AnchoredEnsemble(pl.LightningModule):
    def __init__(self, config):
        ...
        self.net = Ensemble(
            mlp_input_dim=m['input_dim'],
            mlp_hidden_dim=m['hidden_dim'],
            mlp_output_dim=m['output_dim'],
            mlp_n_layers=m.get('n_layers', 2),
            mlp_anchored=m.get('anchored', True),
            mlp_l2_lambda=m.get('l2_lambda', 1e-4),
            mlp_n_ensemble=m.get('n_ensemble', 10)
        )
    ...
    def training_step(self, batch, _):
        x, y = batch
        logprobs_N_K_C, loss = self.net(x, y)
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=0):
        x, y = batch
        logprobs_N_K_C, _ = self.net(x)
        
        # Average log probabilities across ensemble members and convert to logits
        avg_logprobs = logprobs_N_K_C.mean(dim=1)  # N K C -> N C
        return avg_logprobs
...
class MLP(nn.Module):
    ...
    def forward(self, x: Tensor, y: Tensor = None) -> tuple[Tensor, Tensor, Tensor]:
...
def anchored_loss(model: MLP, x: Tensor, y: Tensor = None) -> tuple[Tensor, Tensor]:
